...
{
	long numTosses, numInCircle, numThreadTosses, totalNumTosses, numberInCircle, my_thread_id;
	double piEstimate;
	clock_t start, finish;
	double PI25DT = 3.141592653589793238462643; /* 25-digit-PI*/

	omp_set_num_threads(8);			   // Set number of threads
	numThreadTosses = totalNumTosses / 8; // Calculate number of tosses per thread
	...
	#pragma omp parallel reduction(+: numberInCircle)
	{
		...
		numberInCircle = Toss(numThreadTosses);
	}
